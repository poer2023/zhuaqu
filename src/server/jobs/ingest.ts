...
type IngestFailure = { url: string; code: string; error: string }
...
export async function runIngestJob(jobId: string): Promise<void> {
  ...
  for (const rawUrl of urls) {
    ...
    if (!parsed) {
      failed++
      failures.push({ url: rawUrl, code: "INVALID_URL", error: "Invalid X/Twitter URL" })
      await prisma.ingestJob.update({ where: { id: jobId }, data: { failed, failures } })
      continue
    }

    const existing = await prisma.contentItem.findUnique({
      where: { workspaceId_sourceId: { workspaceId: job.workspaceId, sourceId: parsed.tweetId } },
      select: { id: true },
    })
    if (existing) {
      if (tagIds.length > 0) {
      ...
      if (mediaMode === "download") {
        try {
          ...
          failed++
          failures.push({ url: rawUrl, code: getErrorCode(error), error: getErrorMessage(error) })
          await prisma.ingestJob.update({ where: { id: jobId }, data: { failed, failures } })
          continue
        }
      }
